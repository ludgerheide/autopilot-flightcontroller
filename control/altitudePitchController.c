...
s08 calculatePitchAngle(s32 targetAltitude) {
    ...
    s16 normalized_integral = maps32(integratedAltitudeError, -maxAltitudeIntegral, maxAltitudeIntegral,
                                     -maxNormalizedIntegral, maxNormalizedIntegral);
    s16 normalized_proportional = maps32(altitudeError, -maxAltitudeProportional, maxAltitudeProportional,
                                         -maxNormalizedProportional, maxNormalizedProportional);
    ...
    float desiredAngle = mapfloat(proportional_integral, -16384, 16384, -MAX_AUTONOMOUS_PITCH_ANGLE,
                                  MAX_AUTONOMOUS_PITCH_ANGLE);
...
u08 calculateElevatorValue(s08 targetPitchAngle) {
    ...
    float normalizedOutput = pitchProportionalNormalized * ELEVATOR_PROPORTIONAL_GAIN +
                             pitchIntegralNormalized * ELEVATOR_INTEGRAL_GAIN +
                             pitchDerivNormalized * ELEVATOR_DIFFERENTIAL_GAIN;
