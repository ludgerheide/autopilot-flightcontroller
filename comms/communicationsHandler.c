...
static u08 createProtobuf(messagePurpose thePurpose, u08 *messageLength) {
    ...
    if ((thePurpose == logging && GpsInfo.VelHS.timestamp - logVelTime) ||
        (thePurpose == telemetry && GpsInfo.VelHS.timestamp - telVelTime)) {
...
void commsProcessMessage(char *message, u08 size) {
    ...
    if (incomingMsg.has_autonomous_update) {
        ...
        if (incomingMsg.autonomous_update.has_altitude) {
            ...
            if (incomingMsg.autonomous_update.pitchAngle <= INT8_MAX &&
                incomingMsg.autonomous_update.pitchAngle >= INT8_MIN) {
        ...
        if (incomingMsg.autonomous_update.has_heading) {
            ...
            if (incomingMsg.autonomous_update.rateOfTurn <= INT8_MAX &&
                incomingMsg.autonomous_update.rateOfTurn >= INT8_MIN) {
